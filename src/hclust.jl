...
Given a hierarchical cluster, and an n x n distance matrix, use fast algorithm
to determine optimal leaf order minimizing the distance between adjacent leaves.
This is done using a heuristic where, when combining multi-leaf sub branches,
only the outermost leaves are compared (a maximum of 4 comparisons per
intersection).
